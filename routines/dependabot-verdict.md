...
**Trigger:** GitHub → Pull request → `labeled`. Filters: Author equals `dependabot[bot]`; Labels is one of `major-review-needed`, `dependabot-needs-human`, `no-ci-gate` — plus `dependabot-ci-failed` in a repo that turns the failed-test watch on.

**Only when turning the failed-test watch on in a repo (added 2026-09-22, OFF by default):** add `dependabot-ci-failed` to that repo's routine's "Labels is one of" filter, as one of the three steps in the README ("A queued update whose tests go red"). Nothing else changes: the prompt below already reads the tests and handles "Merge after a fix". With the watch off, red queued updates reach Slack through the Monday digest's open loops instead. (Labels the workflow applies do fire these routines, even though they come from `GITHUB_TOKEN`: the workflow-labelled majors on 2026-09-16 got their verdicts the same day. The "token pushes start no workflow" rule is about Actions runs, not about the Claude GitHub App's webhooks.)
