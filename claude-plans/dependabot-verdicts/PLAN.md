...
4. Every labelled PR produces one plain-English verdict in #dependabot within minutes, in the seven-line shape, ending with the exact reply to send. ✅ proven on eachie #143 (old shape); the four routines now carry the final shape; ⏳ not yet observed in the final shape on a real PR.
5. A weekly digest in #dependabot: open Dependabot PRs by repo with age and label, plus drift (repo missing stub / auto-merge / required check / approve switch; a job without timeout-minutes; the Actions-minutes estimate). ⏳ this is the overnight build (repo-standards#1 is the brief).
6. Nothing in the standard needs Kevin's Mac to be on. ✅ (Actions + Anthropic cloud).
7. Cost stays inside the free Actions pool most months; the $48 budget is the backstop. ✅ set; ⏳ digest reports the burn.
8. Kevin's channel tidy (#infra-ops, #eachie-digest, archive the dead ones). ⏳ needs a Slack-signed-in browser or Kevin's clicks; click list in NOW.md.
